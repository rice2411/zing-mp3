...
const Carousel = ({ props = null }: any) => {
    ...
    const slide = (next = true) => {
        setActiveItem((pre) => {
            return next ? (pre + carouselList.length - 1) % carouselList.length : (pre + 1) % carouselList.length;
        });
    };
    ...
    return (
      <div>
          ...
          >
            {
              !buttonHidden && 
              <button className="slider-btn slider-pre-btn" onClick={() => slide(false)}>
              ...
              {
                !buttonHidden &&
                <button className="slider-btn slider-next-btn" onClick={() => slide(true)}>
