...
function AdminAlbum() {
  ...
  const fetchData = async (params: any) => {
    ...
    try {
      const response: any = await AlbumService.getAll(param);
  ...
  const handleDeleteAlbum = (albumId: string) => {
    handleModalConfirm({
      text: {
        title: "Xoá album?",
        description: "Hành động của bạn sẽ xoá album này vĩnh viễn",
      },
      isShow: true,
      onAction: () => {
        deleteAlbum(albumId);
  ...
  const deleteAlbum = async (albumId: string) => {
    try {
      const response: any = await AlbumService.delete(albumId);
      if (response?.data?.data) {
        handleModalConfirm({
          isShow: false,
          onSubmit: null,
        });
        toast("🦄 Xoá album thành công!", {
          position: "top-right",
          autoClose: 5000,
  ...
  const handleModifyArtist = async (values: any) => {
    ...
    try {
      const response: any =
        editData != null
          ? await AlbumService.update(formData)
          : await AlbumService.create(formData);
      if (response?.data?.data) {
        handleModalBlank({
          isShow: false,
        });
        toast(`🦄 ${editData ? "Chỉnh sửa" : "Thêm mới"} album thành công!`, {
          position: "top-right",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
